...
**Q4 (3pnts) Consider the evapotranspiration module and the discussion potential and actual evapotranspiration. Why do you think PET yields estimates of negative residual water in this high elevation, semi-arid system?**
ANSWER:
...
Now, let's see how well we can get it to work. The code below runs the model, produces a plot, and calculates the NSE and KGE based on discharge.

**Q5. (5 pts) By changing the parameters, see how well you can get the model to fit. Take note of how the discharge changes when you change certain parameters. The parameter ranges in the comments are just suggestions, so you can go higher or lower (if it makes physically sense). Let's see who can get the highest NSE!**
...
**Q6. (3 pts) What do the parameters do? In other words, what do they represent in the model? What might be an effective alternative to manually calibrating your model parameters?**
...
q_plot <- BLANK +
  ...
  BLANK(aes(x = Date, y = Qsim, color = "Qsim")) + # Qsim
